...
class BeanCalculationNetWorkerTest: XCTestCase {
    ...
    func test_baseWith_0quantity(){
        let subtotal = BeanCalculWorker.calculSubTotal(base: 100, quantity: 0)
        XCTAssertEqual(subtotal, 0)
    }
    func test_baseWith_1quantity(){
        let subtotal = BeanCalculWorker.calculSubTotal(base: 100, quantity: 1)
        XCTAssertEqual(subtotal, 100)
    }
    func test_baseWith_2quantity(){
        let subtotal = BeanCalculWorker.calculSubTotal(base: 100, quantity: 2)
    ...
    func test_baseWith_1quantity_taxe50Percent(){
        let net = BeanCalculWorker.calculTotalNet(base: 100, quantity: 3, tax: 19.6)
